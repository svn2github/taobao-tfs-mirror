...
namespace tfs
{
  namespace nameserver
  {
    ...
    TEST_F(FamilyManagerTest, MarshallingItem_get)
    {
      ...
      std::pair<uint64_t, uint32_t> out;
    ...
    TEST_F(FamilyManagerTest, insert_remove_get)
    {
      ...
      std::pair<uint32_t, int32_t> item;
      ...
      std::pair<uint32_t, int32_t> results[MEMBER_NUM];
      common::ArrayHelper<std::pair<uint32_t, int32_t> > helper2(MEMBER_NUM, results);
      ...
      for (int32_t i = 0; i < MEMBER_NUM; ++i)
      {
        ...
        std::pair<uint32_t, int32_t> item2 = *helper2.at(i);
    ...
    TEST_F(FamilyManagerTest, scan)
    {
      ...
      for (int32_t k = 0; k < MAX_FAMILY_NUM / MAX_QUERY_NUM; k++)
      {
        ...
        for (int32_t m = 0; m < result.get_array_index(); ++m)
        {
          ...
          std::pair<uint32_t, int32_t> results2[MEMBER_NUM];
          common::ArrayHelper<std::pair<uint32_t, int32_t> > helper2(MEMBER_NUM, results2);
          family->get_members(helper2);
          for (int32_t n = 0; n < helper2.get_array_index(); ++n)
          {
            std::pair<uint32_t, int32_t> item2 = *helper2.at(n);
            EXPECT_EQ((family_id + n), item2.first);
    ...
    TEST_F(FamilyManagerTest, create_family_choose_data_members)
    {
      ...
      common::ArrayHelper<std::pair<uint64_t, uint32_t> > helper(MAX_MARSHALLING_NUM, members);
      ...
      {
        std::pair<uint64_t, uint32_t> item = *helper.at(i);
        for (int32_t j = i + 1; j < helper.get_array_index(); j++)
        {
          std::pair<uint64_t, uint32_t> item2 = *helper.at(j);
