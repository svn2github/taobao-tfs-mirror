...
namespace tfs
{
  namespace dataserver
  {
    ...
    int ClientRequestServer::write_file(WriteFileMessageV2* message)
    {
      ...
      if (TFS_SUCCESS == ret)
      {
        ret = get_op_manager().prepare_op(attach_block_id,
            file_id, lease_id, OP_TYPE_WRITE, servers, true);
        if (TFS_SUCCESS != ret)
        {
    ...
    int ClientRequestServer::close_file(CloseFileMessageV2* message)
    {
      ...
      {
        ret = get_op_manager().prepare_op(attach_block_id,
            file_id, lease_id, OP_TYPE_WRITE, servers, false);
        if (TFS_SUCCESS != ret)
        {
         TBSYS_LOG(WARN, "prepare write lease fail. blockid: %"PRI64_PREFIX"u, "
    ...
    int ClientRequestServer::unlink_file(UnlinkFileMessageV2* message)
    {
      ...
      {
        ret = get_op_manager().prepare_op(attach_block_id,
            file_id, lease_id, OP_TYPE_UNLINK, servers, prepare);
        if (TFS_SUCCESS != ret)
        {
      ...
      if (TFS_SUCCESS == ret)
      {
        if (prepare)
        {
          ret = get_op_manager().prepare_unlink_file(block_id,
    ...
    int ClientRequestServer::write_file_callback(WriteFileMessageV2* message)
    {
      ...
      if (all_finish)
      {
        ...
        if (TFS_SUCCESS != ret)
        {
          ...
          {
            if (TFS_SUCCESS != get_op_manager().resolve_block_version_conflict(attach_block_id, file_id, lease_id))
            {
