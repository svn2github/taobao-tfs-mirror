      ...
      if (TFS_SUCCESS == ret)
      {
        ...
        if (TFS_SUCCESS == ret)
        {
          ...
          if (TAIR_RETURN_SUCCESS == ret)
          {
            ...
            TBSYS_LOG(DEBUG, "deserialize value. value len: %d, version: %d", value_len, value.version_);
    ...
    int TairCacheHelper::put(const BlockCacheKey& key, const BlockCacheValue& value)
    {
      ...
      if (TFS_SUCCESS == ret)
      {
        ...
        if (TFS_SUCCESS == ret)
        {
          ...
          {
            TBSYS_LOG(ERROR, "serialize value fail. ret: %d", ret);
          }
          ...
          if (TFS_SUCCESS == ret)
          {
            ...
            if (TAIR_RETURN_SUCCESS == ret || TAIR_RETURN_PARTIAL_SUCCESS == ret)
            {
              ...
              for (; kv_entries.end() != kv_entries_iter;)
              {
                ...
                TBSYS_LOG(DEBUG, "deserialize key. key len: %d", get_key_len);
                pos = 0;
                ...
                if (TFS_SUCCESS == ret)
                {
                  ...
                  TBSYS_LOG(DEBUG, "deserialize value. value len: %d, version: %d", get_value_len, blk_cache_value.version_);
