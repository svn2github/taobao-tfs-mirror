...
namespace tfs
{
  namespace nameserver
  {
    ...
    int MetaManager::join_ds(const DataServerStatInfo& dsInfo, bool& isnew)
    {
      int iret = meta_mgr_.join_ds(dsInfo, isnew);
      if (iret != TFS_SUCCESS)
      {
        TBSYS_LOG(ERROR, "ds(%s) join faild", tbsys::CNetUtil::addrToString(dsInfo.id_).c_str());
        return iret;
      }
      meta_mgr_.update_global_info(dsInfo, isnew);
      return TFS_SUCCESS;
    }

    uint32_t MetaManager::register_expire_block(EXPIRE_BLOCK_LIST & expireList, const uint64_t ds_id,
        const uint32_t block_id)
    {
      EXPIRE_BLOCK_LIST::iterator it = expireList.find(ds_id);
      if (it != expireList.end())
        it->second.push_back(block_id);
      else
        expireList.insert(EXPIRE_BLOCK_LIST::value_type(ds_id, vector<uint32_t> (1, block_id)));
      return expireList.size();
    }
