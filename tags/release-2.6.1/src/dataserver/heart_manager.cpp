  ...
  namespace dataserver
  {
    ...
    int DataServerHeartManager::keepalive(int8_t& heart_interval, const int32_t who, const int64_t timeout)
    {
      ...
      if (TFS_SUCCESS == ret)
      {
        ...
        TBSYS_LOG(INFO, "KEEPALIVE, who: %d, write bytes: %ld,  write count: %ld, unlink count %ld",
            who, info.write_bytes_[0], info.write_file_count_[0], info.unlink_file_count_[0]);
