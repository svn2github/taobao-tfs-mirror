...
public class NameServerPlanTestCase {
	...
	final public double NETWORKTRAF_IMBALANCE_THRESHOLD    = 128000.0;
...
//				break;
	 ...
	 * @return
	 */
	public boolean chkNetworkTrafBalance()
	{	
		boolean bRet = false;
		
		String strCmd = "sar -n DEV -f " + SAR_DSA_LOG + 
			" |grep Average | grep eth0 | awk '{print $" + RXBYTPERSEC_SD_COL + "}'";
		ArrayList<String> output = new ArrayList<String>();
		...
		if (output.size() == 0)
			return false;
		double rxbytPerSecDSA = Double.parseDouble(output.get(0));	
		...
		if (output.size() == 0)
			return false;
		double rxbytPerSecDSB = Double.parseDouble(output.get(0));	
		...
		double diff = rxbytPerSecDSA - rxbytPerSecDSB;
