...
TEST_F(TfsInit,02_save_file_right_small_wrong_flag)
{
   ...
   Ret=tfsclient->save_file(a100K, tfs_name, tfs_name_len,true);
   EXPECT_EQ(100*(1<<10),Ret);
...
TEST_F(TfsInit,03_save_file_right_large)
{
   ...
   Ret=tfsclient->save_file(a1G, tfs_name, tfs_name_len,true);
   EXPECT_EQ(1<<30,Ret);
}
...
TEST_F(TfsInit,11_save_file_small_wrong_tfs_name_len)
{
   char tfs_name[19];	 
...
TEST_F(TfsInit,12_save_file_small_mroe_tfs_name_len)
{
   char tfs_name[19];	 
