...
public class NetworkUtility 
{
	...
	{
		boolean ret = false;
		String cmd = "/sbin/iptables -D OUTPUT -p tcp -d " + freeIp + " --dport " + port + " -j DROP;/sbin/iptables-save";
		...
		if (result.isSuccess()) 
		{
			logger.debug("Network from " + tarIp + " to " + freeIp + " is freed on port " + port);
			ret = true;
		} 
		else 
		{
			logger.error("Network from " + tarIp + " to " + freeIp + " is failed to be freed on port " + port);
			ret = false;
		}
	...
	{
		boolean ret = false;
		String cmd = "/sbin/iptables -A OUTPUT -p tcp -d " + blkIp + " --dport " + port + " -j DROP;/sbin/iptables-save";
		...
		if (result.isSuccess()) 
		{
			logger.debug("Network from " + tarIp + " to " + blkIp + " is blocked on port " + port);
			ret = true;
		} 
		else 
		{
			logger.error("Network from " + tarIp + " to " + blkIp + " is failed to be blocked on port " + port);
			ret = false;
		}
	...
	public static boolean blockNetwork(String tarIp, String blkIp) 
	{
		...
		boolean bRet = false;
		String strCmd = "/sbin/iptables -A OUTPUT -d " + blkIp + " -j DROP;/sbin/iptables-save";
		...
		if (result.isSuccess()) 
		{
			logger.debug("Network from " + tarIp + " to " + blkIp + " is blocked!");
			bRet = true;
		} 
		else 
		{
			logger.error("Network from " + tarIp + " to " + blkIp + " is failed to block!!!");
			bRet = false;
		}
