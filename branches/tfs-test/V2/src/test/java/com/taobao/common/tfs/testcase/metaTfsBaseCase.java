...
public class metaTfsBaseCase extends BaseCase 
{
	...
	/*______________________参量配置_____________________________________*/
    ...
    /*__________________________读配置文件_________________________________*/ 
    ...
    /*__________________________使用mysql代替_________________________________________*/
    ...
    final public int COL_FAIL_COUNT = 19;
    ...
    protected final static String CONF_CREATE_DIR = "CreateDir";
    ...
    protected final static String CONF_SAVE_SMALL_FILE = "SaveSmallFile";
	...
	/*____________________留白_______________________________*/
	...
	/*___________________________函数区___________________________*/
    ...
    /*_________________________函数区一区结束____________________________________*/
    ...
    /*_________________________函数区二区____________________________________*/
   ...
   public boolean chkRateEnd(float std, int operType, long userId) {
    ...
    public float getRateRun(String tarIp, String fileName, String keyWord) {
      ...
      float failCount = result.get(result.size() - 1) - result.get(result.size() - 2);
      if (failCount == 0) {
        fRet = 100;
      }
      return fRet;  
    ...
    public boolean chkRateRun(float std, int operType, long userId) {
        ...
        bRet = Proc.proStartBack(CLIENT_IP, cmd + " > " + tmpLog);
        if (bRet == false) return bRet;

        /* Wait */
        sleep(SCAN_TIME);
        
        bRet = Proc.proStopByCmd(CLIENT_IP, cmd);
        if (bRet == false) return bRet;
...
//mazhentong.pt
    public boolean chkRateByRun(int operType, long userId) {
        boolean bRet = false;
        String sorLog = CLIENT_LOG + caseName + "." + userId;
        String tmpLog = CLIENT_LOG + caseName + ".tmp." + userId;
        String cmd = "tail -f " + sorLog;
	...
	if( result.size() > 0 )
	{
	    failCount = Integer.parseInt(result.get(result.size()-1));
	}
	/*get success count*/
	int succCount = 0;
	cmd = "grep \"success\" " + tmpLog + " | wc -l";
        ...
        if( result.size() > 0 )
        {
            succCount = Integer.parseInt(result.get(result.size()-1));
        }

	if(0 == (failCount + succCount) )
	{
 	   log.error("create dir failCount: "+ failCount + " , succCount: "+ succCount + " !!!");
	   return false;
	}
	if(0 == failCount)
	{
	   log.info("create dir failCount: "+ failCount + " , succCount: "+ succCount + " !!!");
	   return true;
	}
	else
	{
...
//mazhentong.pt
    public boolean resetAllFailCount()
    {
	return resetAllFailCnt(MASTER_RS,SLAVE_RS);
    }
//mazhentong.pt
