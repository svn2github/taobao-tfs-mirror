CREATE database if not exists `tfs_stat`;

USE tfs_stat;

CREATE TABLE if not exists `t_resource_server_info` (
...
CREATE TABLE if not exists `t_meta_root_info` (
  app_id INT,
  addr_info VARCHAR(64) NOT NULL,
  stat INT NOT NULL DEFAULT 1,
  rem VARCHAR(255),
...
CREATE TABLE if not exists `t_cluster_rack_info` (
  cluster_rack_id INT,
...
CREATE TABLE if not exists `t_cluster_rack_group` (
...
CREATE TABLE if not exists `t_cluster_rack_duplicate_server` (
  cluster_rack_id INT,
...
CREATE TABLE if not exists `t_base_info_update_time` (
...
CREATE TABLE if not exists `t_session_stat` (
  session_id VARCHAR(255),
  oper_type  INT,
  oper_times BIGINT,
  file_size BIGINT,
...
CREATE TABLE if not exists `t_app_stat` (
  app_id INT,
...
CREATE TABLE if not exists `t_caculate_ip_info` (
  source_ip VARCHAR(64),
  caculate_ip VARCHAR(64),
  PRIMARY KEY (source_ip)
);
CREATE TABLE if not exists `t_app_ip_replace` (
  app_id INT,
  source_ip VARCHAR(64),
  turn_ip VARCHAR(64),
...
CREATE TABLE  if not exists `t_cluster_cache_info` (
