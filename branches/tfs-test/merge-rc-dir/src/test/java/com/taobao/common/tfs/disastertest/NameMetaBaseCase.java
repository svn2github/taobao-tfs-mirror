...
public class NameMetaBaseCase extends TfsBaseCase{
    ...
    public String SLAVE_RS_IP = nameMetaGrid.getCluster(RSINDEX).getServer(1).getIp();
    ...
    // ha related
    final public String VIP_ETH_NAME = "eth0:0";
    ...
    final public String CREATEDIR_LOG = "createdir_log.";
    final public String CREATEDIR_CMD = "sh catRate ";
    @BeforeClass
...
//mazhentong.pt
    public boolean createDirCmdStart(){
	boolean bRet = false;
	log.debug("Create dir cmd start ===>");
	String cmd = "sh start log." + caseName;
	bRet = Proc.proStartBackroundBase(CLIENT_IP, cmd, CLIENT_HOME);
	log.debug("Create dir cmd end ===>");
	return bRet;
    }
//mazhentong.pt
    public boolean createDirCmdEnd() {
        boolean bRet = false;
...
//mazhentong.pt
    public boolean cleanUpAllHA(){

	log.info("Clean up all HA start ===>");
        boolean bRet = cleanUpHA(MASTER_RS);
        if (bRet == false) return bRet;

        bRet = cleanUpHA(SLAVE_RS);
        if (bRet == false) return bRet;
        log.info("Clean up all HA end ===>");
        return bRet;

    }
//mazhentong.pt
    public boolean startAllRS(){
	boolean bRet = false;
	log.debug("Start all rootserver start ===>");
	bRet = MASTER_RS.start();
	if (false == bRet) return false;
        bRet = SLAVE_RS.start();
	log.debug("Start all rootserver end ===>");
        return bRet;
    }
//mazhentong.pt
    public boolean isCreateLog(){
	boolean bRet = false;
	String cmd = CREATEDIR_CMD + " log." + caseName + " " + CREATEDIR_LOG + caseName;
	bRet = Proc.proStartBase(CLIENT_IP, cmd, CLIENT_HOME);
	return bRet;
    }
//mazhentong.pt
    public boolean isCreateLogMin(){
	boolean bRet = false;
	for(;;) {
          int iRet = Proc.proMonitorBase(CLIENT_IP, CREATEDIR_CMD);
          if (0 == iRet) {
             bRet = true;
             break;
          }
             ...
             bRet = false;
	     log.error("process monitor("+ CREATEDIR_CMD +") Failed");
             break;
          }
        }
...
//mazhentong.pt
    public boolean chkFailRate(){
	float actRate = -1;
        actRate = getFailRate();

        if (actRate == -1) {
            log.error("get fail rate Failed !!!");
            return false;
        }
        if (actRate != 0) {
            log.error("create dir fail rate is not 0% !!!");
            return false;
        }
        else {
            log.info("create dir fail rate is 0% !!!");
        }
        return true;
...
//mazhentong.pt
    public boolean chkSucRate(float expectRate){
	float actRate = 0;
	actRate = getSucRate();

	if (actRate == -1) {
	    log.error("get success rate Failed !!!");
            return false;
        }
        if (actRate < expectRate) {
            log.error("create dir success rate(" + actRate + "%) is less than " + expectRate + "% !!!");
            return false;
        }
        else {
            log.info("create dir success rate(" + actRate + "%) is not less than " + expectRate + "% !!!");
        }
        return true;
...
//mazhentong.pt
	...
	boolean bRet = false;
	log.info("get success rate start ===>");
	float iRet = -1;
	String fileName = CLIENT_HOME +"/"+ CREATEDIR_LOG + caseName;
	String keyWord = "successRate(%): ";
//	String keyword = "failRate(%):";
	ArrayList<String> result = new ArrayList<String>();
	bRet = Log.scanAllBase(CLIENT_IP, fileName, keyWord, 7, null, result);
        if ((bRet == false) || (result.size() < 1))
        {
           return iRet;
        }
        String str = result.get(result.size() - 1);
	iRet = Float.parseFloat(str);
	log.info("get success rate end ===>");
	return iRet;
    }
//mazhentong.pt
    public float getFailRate(){
        boolean bRet = false;
        log.info("get fail rate start ===>");
        float iRet = -1;
        String fileName = CLIENT_HOME +"/"+ CREATEDIR_LOG + caseName;
        String keyword = "failRate(%):";
        ArrayList<String> result = new ArrayList<String>();
        ...
        String str = result.get(result.size() - 1);
	log.error("fail rate: "+ str);
        iRet = Float.parseFloat(str);
        log.info("get fail rate end ===>");
        return iRet;
    }
