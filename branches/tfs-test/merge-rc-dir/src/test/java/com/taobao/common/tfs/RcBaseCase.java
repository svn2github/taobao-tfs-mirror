...
public class RcBaseCase extends TfsBaseCase {
	...
	final public String MYSQLADDR = "10.232.36.208";
	...
	public void addNewTfsCluster(String appKey, int index, String clusterName) {
		...
		String nsAddr = nsIp + ":" + port;
		String cmd;
		cmd = "sh " + SCRIPT_PATH + " add_cluster " + appKey + " "
				+ clusterName + " " + nsAddr;
		bRet = Proc.cmdOutBase(MYSQLADDR, cmd, null, 1, null, listOut);
	}
	...
	public boolean checkSessionId(String sessionId) {
		...
		ArrayList<String> listOut = new ArrayList<String>();
		String cmd;
		cmd = "sh " + SCRIPT_PATH + " get_session_size '" + sessionId + "'";
		bRet = Proc.cmdOutBase(MYSQLADDR, cmd, null, 1, null, listOut);
		if (bRet) {
	...
	public boolean setDuplicateStatus(String appKey, int status) {
		...
		ArrayList<String> listOut = new ArrayList<String>();
		String cmd;
		cmd = "sh " + SCRIPT_PATH + " set_duplicate_status " + appKey + " "
				+ status;
		bRet = Proc.cmdOutBase(MYSQLADDR, cmd, null, 1, null, listOut);
		return bRet;
	}

	public boolean changeDuplicateServerAddr(int clusterRackID,
			String duplicateServerAddr) {
		boolean bRet = false;
		ArrayList<String> listOut = new ArrayList<String>();
		String cmd;
		cmd = "sh " + SCRIPT_PATH + " change_duplicate_server_addr "
				+ clusterRackID + " '" + duplicateServerAddr + "'";
		bRet = Proc.cmdOutBase(MYSQLADDR, cmd, null, 1, null, listOut);
		return bRet;
	}
	...
	public long getFileSize(String sessionId, int operType) {
		...
		ArrayList<String> listOut = new ArrayList<String>();
		String cmd;
		cmd = "sh " + SCRIPT_PATH + " get_file_size '" + sessionId + "' "
				+ operType;
		bRet = Proc.cmdOutBase(MYSQLADDR, cmd, null, 1, null, listOut);
