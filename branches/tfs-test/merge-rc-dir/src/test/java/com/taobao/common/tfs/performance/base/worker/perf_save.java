...
public class perf_save extends AbstractWorker {
	...
	public boolean do_work() {
		...
		log.info("start to save file: " + path);

		if (bRet == false) {
			log.error("save file fail,the file name is: " + path);
			num++;
			return false;
		}
		log.info("succeed to save file: " + path);
		num++;
		return true;
	}
