...
public class TfsClientTest
{
    ...
    private static TfsManager tfsManager = null;
    ...
    //@Test
    public void TestDir() {
        tfsManager.rmFile(userId, newFileName);
        tfsManager.rmDir(userId, newDirName);
        tfsManager.rmFile(userId, fileName);
        tfsManager.rmDir(userId, dirName);
        assertTrue(tfsManager.createDir(userId, dirName));
        assertFalse(tfsManager.createDir(userId, dirName));
        assertTrue(tfsManager.mvDir(userId, dirName, newDirName));
        assertTrue(tfsManager.rmDir(userId, newDirName));
    }

    //@Test
    public void TestFile() {
        assertFalse(tfsManager.createDir(userId, "/ "));
        tfsManager.createDir(userId, "/text1");
        tfsManager.createDir(userId, "/text1/text2");
        tfsManager.createDir(userId, "/text1/text2/text3");
        assertFalse(tfsManager.mvDir(userId, "/text1", "/text1/text2/text3/text4"));
        tfsManager.rmDir(userId, "/text1/text2/text3");
        tfsManager.rmDir(userId, "/text1/text2");
        tfsManager.rmDir(userId, "/text1");
        ...
        assertTrue(tfsManager.mvDir(userId, dirName, newDirName));
        assertFalse(tfsManager.mvDir(userId, dirName, newDirName));
        assertTrue(tfsManager.rmDir(userId, newDirName));

        assertFalse(tfsManager.createFile(userId, fileName));
        assertTrue(tfsManager.createDir(userId, dirName));
        assertTrue(tfsManager.createFile(userId, fileName));
        assertFalse(tfsManager.createFile(userId, fileName));
        assertFalse(tfsManager.mvFile(userId, fileName, newFileName));
        assertTrue(tfsManager.createDir(userId, newDirName));

        assertTrue(tfsManager.mvFile(userId, fileName, newFileName));
        assertFalse(tfsManager.mvFile(userId, fileName, newFileName));

        assertTrue(tfsManager.rmFile(userId, newFileName));
        assertFalse(tfsManager.rmFile(userId, newFileName));
        assertTrue(tfsManager.rmDir(userId, dirName));
        assertTrue(tfsManager.rmDir(userId, newDirName));
        assertFalse(tfsManager.rmDir(userId, newDirName));
    }
    ...
    public void TestAction() {
        ...
        assertTrue(tfsManager.createDir(userId, dirName));
        assertTrue(tfsManager.createFile(userId, fileName));
        assertTrue(tfsManager.rmFile(userId, fileName));
        assertTrue(tfsManager.rmDir(userId, dirName));
    }
    ...
    public void TestWrite() {
        //for (int i = 0; i < 10; i++) {
            ...
            assertEquals(tfsManager.write(userId, fileName, 0, data, 0, length), length);
            assertEquals(tfsManager.write(userId, fileName, length + 1, data, 0, length), length);
            assertEquals(tfsManager.write(userId, fileName, length, data, 0, 1), 1);
            try {
                FileOutputStream fos = new FileOutputStream(smallFileRet.getFileName());
                assertEquals(tfsManager.read(userId, fileName, 0, length, fos), length);
                assertEquals(tfsManager.read(userId, fileName, 1, length - 1, fos), length - 1);
                assertEquals(tfsManager.read(userId, fileName, 0, 2* length, fos), 2 * length);
                assertEquals(tfsManager.read(userId, fileName, 1, 2* length, fos), 2 * length);
                assertEquals(tfsManager.read(userId, fileName, length + 2, length, fos), length - 1);
                assertEquals(tfsManager.read(userId, wrongFileName, 1, 2 * length, fos), -1);
                assertEquals(tfsManager.read(userId, newFileName, 1, 2 * length, fos), -1);
                //assertEquals(tfsManager.read(userId, fileName, length, length, fos), length);
            //assertEquals(tfsManager.read(userId, fileName, length*2, length, fos), 0);
    ...
    //@Test
      public void test_20_write_large_many_times_parts()  throws IOException {
        ...
        Ret=tfsManager.write(userId, "/text", offset,data, dataOffset, len);
        assertTrue(Ret==0);
        tfsManager.fetchFile(userId, "temp", "/text");
    ...
    public void TestWriteFile() {
        ...
        tfsManager.rmFile(userId, fileName);
        tfsManager.rmDir(userId, dirName);
        assertFalse(tfsManager.saveFile(userId, name, fileName));
        assertTrue(tfsManager.createDir(userId, dirName));
        assertTrue(tfsManager.saveFile(userId, name, fileName));
        assertTrue(tfsManager.fetchFile(userId, retName, fileName));
        assertFalse(tfsManager.fetchFile(userId, retName, wrongFileName));
        assertFalse(tfsManager.fetchFile(userId, retName, newFileName));
        assertTrue(tfsManager.rmFile(userId, fileName));
        assertTrue(tfsManager.rmDir(userId, dirName));
    }
    ...
    public void Testls() {
        for (int i = 0; i < 4; i++) {
            tfsManager.rmDir(userId, dirName + "/" + i + "/" + i + "dir");
            tfsManager.rmDir(userId, dirName + "/" + i + "/" + i + "dir1");
            tfsManager.rmFile(userId, dirName + "/" + i + "/" + i*5);
            tfsManager.rmDir(userId, dirName + "/" + i);
            tfsManager.rmFile(userId, fileName + i);
        }
        tfsManager.rmFile(userId, fileName);
        tfsManager.rmDir(userId, dirName);
        assertTrue(tfsManager.createDir(userId, dirName));
        ...
        //TestWriteFile();
        for (int i = 0; i < 7; i++) {
            assertTrue(tfsManager.createFile(userId, fileName + i));
            assertTrue(tfsManager.createDir(userId, dirName + "/" + i));
            assertTrue(tfsManager.createFile(userId, dirName + "/" + i + "/" + i*5));
            assertTrue(tfsManager.createDir(userId, dirName + "/" + i + "/" + i + "dir"));
            assertTrue(tfsManager.createDir(userId, dirName + "/" + i + "/" + i + "dir1"));
        }
        FileMetaInfo fileMetaInfo1 = tfsManager.lsFile(userId, fileName);
        assertTrue(fileMetaInfo1 != null);
        log.info("== file " + fileName + fileMetaInfo1);
        for (int i = 0; i < 7; i++) {
            FileMetaInfo fileMetaInfo = tfsManager.lsFile(userId, fileName+i);
            assertTrue(fileMetaInfo != null);
            log.info("== file " + fileName + i + " " + fileMetaInfo);
            fileMetaInfo = tfsManager.lsFile(userId, dirName + "/" + i + "/" + i*5);
            assertTrue(fileMetaInfo != null);
            log.info("== file " + dirName + "/" + i + "/" + i*5 + " " + fileMetaInfo);
        }

        List<FileMetaInfo> metaInfoList = tfsManager.lsDir(userId, dirName);
        ...
        metaInfoList = tfsManager.lsDir(userId, dirName, true);
        assertTrue(metaInfoList != null);
        for (FileMetaInfo metaInfo : metaInfoList)  {
            log.info("[ " + metaInfo + " ]");
        }
        ...
        metaInfoList = tfsManager.lsDir(userId, "/", true);
        assertTrue(metaInfoList != null);
        for (FileMetaInfo metaInfo : metaInfoList)  {
            log.info("[ " + metaInfo + " ]");
        }
        ...
        for (int i = 0; i < 7; i++) {
            assertTrue(tfsManager.rmDir(userId, dirName + "/" + i + "/" + i + "dir"));
            assertTrue(tfsManager.rmDir(userId, dirName + "/" + i + "/" + i + "dir1"));
            assertTrue(tfsManager.rmFile(userId, dirName + "/" + i + "/" + i*5));
            assertTrue(tfsManager.rmDir(userId, dirName + "/" + i));
            assertTrue(tfsManager.rmFile(userId, fileName + i));
        }
        assertTrue(tfsManager.rmFile(userId, fileName));
        assertTrue(tfsManager.rmDir(userId, dirName));
    }
