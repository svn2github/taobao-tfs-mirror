...
static void ngx_http_tfs_dedup_get_handler(ngx_http_tair_key_value_t *kv, ngx_int_t rc, void *data);
...
static void ngx_http_tfs_dedup_callback(ngx_http_tfs_dedup_ctx_t *ctx, ngx_int_t rc);

ngx_int_t ngx_http_tfs_dedup_check_suffix(ngx_str_t *tfs_name, ngx_str_t *suffix);
ngx_int_t ngx_http_tfs_dedup_check_filename(ngx_str_t *dup_name, ngx_http_tfs_raw_fsname_t* fsname);
    ...
    ngx_pool_t *pool, ngx_log_t * log)
{
    ...
    *(uint32_t *) p = htonl(data_len);
    ...
    rc = ngx_http_tfs_tair_get_helper(ctx->tair_instance, pool, log,
                                      &tair_key,
                                      ngx_http_tfs_dedup_get_handler, ctx);
...
static void
ngx_http_tfs_dedup_get_handler(ngx_http_tair_key_value_t *kv, ngx_int_t rc, void *data)
{
    u_char                     *p;
    ngx_http_tfs_t             *t;
    ...
    ngx_pool_t *pool, ngx_log_t * log)
{
    ...
    rc = ngx_http_tfs_tair_delete_helper(ctx->tair_instance, pool, log,
                                         &tair_keys,
                                         ngx_http_tfs_dedup_remove_handler, ctx);
...
ngx_int_t
ngx_http_tfs_dedup_check_filename(ngx_str_t *dup_file_name, ngx_http_tfs_raw_fsname_t* fsname)
{
    ngx_int_t                    rc;
...
ngx_http_tfs_dedup_callback(ngx_http_tfs_dedup_ctx_t *ctx, ngx_int_t rc)
{
    ...
    case NGX_HTTP_TFS_ACTION_DEL_OBJECT:
        ...
        case NGX_HTTP_TFS_STATE_REMOVE_READ_META_SEGMENT:
            ...
            if (rc == NGX_OK) {
                /* check file name */
                rc = ngx_http_tfs_dedup_check_filename(&ctx->dup_file_name, &t->r_ctx.fsname);
                if (rc == NGX_OK) {
                    /* file name match, modify ref count and save tair */
                    if (t->r_ctx.unlink_type == NGX_HTTP_TFS_UNLINK_DELETE) {
                        if (--ctx->file_ref_count <= 0) {
                            /* CAUTION: due to history reason, ref count is not precise on production system */
                            /* thus do not remove key from tair and do not unlink file */
                            /* the following right code is commented */

                            /*************** right code start from here *************/
                            ...
                            // rc = ngx_http_tfs_dedup_remove(ctx, t->pool, t->log);
                            // /* do not care delete tair fail, go on unlinking file */
                            // if (rc == NGX_ERROR) {
                            //     ngx_http_tfs_finalize_state(t, NGX_OK);
                            // }
                            /**************** right code end at here ***************/

                            /**************** wrong code start at here ***************/
    ...
    case NGX_HTTP_TFS_ACTION_PUT_OBJECT:
        ...
        case NGX_HTTP_TFS_STATE_WRITE_GET_BLK_INFO:
            ...
            if (rc == NGX_OK) {
                /* check suffix */
                rc = ngx_http_tfs_dedup_check_suffix(&ctx->dup_file_name, &t->r_ctx.file_suffix);
                if (rc == NGX_OK) {
