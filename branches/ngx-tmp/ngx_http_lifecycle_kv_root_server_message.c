...
ngx_chain_t *
ngx_http_lifecycle_root_server_create_message(ngx_pool_t *pool)
{
    ngx_buf_t                  *b;
    ngx_chain_t                *cl;
    ngx_http_lifecycle_kv_rs_request_t  *req;

    b = ngx_create_temp_buf(pool, sizeof(ngx_http_lifecycle_kv_rs_request_t));
    if (b == NULL) {
        return NULL;
    }

    req = (ngx_http_lifecycle_kv_rs_request_t *) b->pos;
    req->header.flag = NGX_HTTP_LIFECYCLE_PACKET_FLAG;
    req->header.len = sizeof(uint8_t);
    req->header.type = NGX_HTTP_LIFECYCLE_REQ_KV_RT_GET_TABLE_MESSAGE;
    ...
    b->last += sizeof(ngx_http_lifecycle_kv_rs_request_t);
...
ngx_int_t
ngx_http_lifecycle_root_server_parse_message(ngx_http_lifecycle_t *t)
{
    u_char                          *p;
    uint16_t                         type;
    ngx_str_t                        action;
    ...
    rc = ngx_http_lifecycle_deserialize_kv_meta_table(&p, &t->loc_conf->meta_server_table);
    if (rc == NGX_ERROR) {
    ...
    t->loc_conf->meta_server_table.valid = NGX_HTTP_LIFECYCLE_YES;
    t->loc_conf->meta_access_count = 0;
    t->loc_conf->meta_fail_count = 0;

    ngx_log_error(NGX_LOG_ERR, t->log, 0,
                  "===========get kv meta table success");
    return NGX_OK;
}
