...
ngx_int_t
ngx_http_tfs_deserialize_authorize_info(u_char **p, ngx_pool_t *pool,
    ngx_http_tfs_authorize_info_t *authorize_info)
{
    ngx_int_t  rc;

    if (p == NULL || *p == NULL || pool == NULL || authorize_info == NULL) {
        return NGX_ERROR;
    }

    rc = NGX_OK;
    /* access_secret_key */
    authorize_info->access_secret_key.len = *((uint32_t *)*p) - 1;
    (*p) += sizeof(uint32_t);

    authorize_info->access_secret_key.data = ngx_pcalloc(pool, authorize_info->access_secret_key.len);
    if (authorize_info->access_secret_key.data == NULL) {
      return NGX_ERROR;
    }

    ngx_memcpy(authorize_info->access_secret_key.data, *p, authorize_info->access_secret_key.len);
    (*p) += authorize_info->access_secret_key.len + 1;

    /* use_name */

    authorize_info->user_name.len = *((uint32_t *)*p) - 1;
    (*p) += sizeof(uint32_t);

    authorize_info->user_name.data = ngx_pcalloc(pool, authorize_info->user_name.len);
    if (authorize_info->user_name.data == NULL) {
      return NGX_ERROR;
    }

    ngx_memcpy(authorize_info->user_name.data, *p, authorize_info->user_name.len);
    (*p) += authorize_info->user_name.len + 1;
    ...
    ngx_http_tfs_bucket_meta_info_t *bucket_meta_info)
{
    ...
    *((uint32_t *)*p) = NGX_HTTP_TFS_BUCKET_META_INFO_HAS_TAG_INFO_TAG;
    (*p) += sizeof(uint32_t);
    bucket_meta_info->has_tag_info = 0;
    *((int8_t *)*p) = bucket_meta_info->has_tag_info;
    (*p) += sizeof(int8_t);

    *((uint32_t *)*p) = NGX_HTTP_TFS_BUCKET_META_INFO_BUCKET_ACL_MAP_TAG;
    (*p) += sizeof(uint32_t);
    /*TODO acl size*/
    *((uint32_t *)*p) = 0;
    (*p) += sizeof(uint32_t);
