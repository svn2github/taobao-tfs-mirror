...
static char *ngx_http_tfs_kvroot_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static char *ngx_http_tfs_tackle_log(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_http_tfs_deprecated_oper_log(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_http_tfs_forbidden_oper_log(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
static ngx_command_t  ngx_http_tfs_commands[] = {
    ...
    { ngx_string("tfs_deprecated_oper_log"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
      ngx_http_tfs_deprecated_oper_log,
      NGX_HTTP_SRV_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("tfs_forbidden_oper_log"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
      ngx_http_tfs_forbidden_oper_log,
      NGX_HTTP_SRV_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("tfs_ignore_client_abort"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_tfs_loc_conf_t, ignore_client_abort),
      NULL },
    ...
    { ngx_string("tfs_intercept_errors"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_tfs_loc_conf_t, intercept_errors),
      NULL },
    ...
    { ngx_string("tfs_kvroot_server"),
      NGX_HTTP_MAIN_CONF|NGX_CONF_TAKE1,
      ngx_http_tfs_kvroot_server,
      0,
      0,
      NULL },

    { ngx_string("tfs_update_kmt_interval_count"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_tfs_loc_conf_t, update_kmt_interval_count),
      NULL },

    { ngx_string("tfs_update_kmt_fail_count"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_tfs_loc_conf_t, update_kmt_fail_count),
      NULL },
...
ngx_http_tfs_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    u.url.len = rc_server->len;
    u.url.data = rc_server->data;
...
static char *
ngx_http_tfs_kvroot_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_tfs_main_conf_t  *tmcf = conf;
...
static char *
ngx_http_tfs_deprecated_oper_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_tfs_srv_conf_t  *tscf = conf;
    ngx_str_t                *value;
...
static char *
ngx_http_tfs_forbidden_oper_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_tfs_srv_conf_t  *tscf = conf;
    ngx_str_t                *value;
...
ngx_http_tfs_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    if (tmcf->body_buffer_size == NGX_CONF_UNSET_SIZE) {
        tmcf->body_buffer_size = (size_t) NGX_HTTP_TFS_DEFAULT_BODY_BUFFER_SIZE;
    }
...
static char *ngx_http_tfs_merge_loc_conf(ngx_conf_t *cf,
    void *parent, void *child)
{
    ngx_http_tfs_loc_conf_t *prev = parent;
    ngx_http_tfs_loc_conf_t *conf = child;
...
ngx_http_tfs_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (tscf->log == NULL) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "in tfs module must assign log path, "
                           "use directives \"tfs_tackle_log\" ");
        return NGX_CONF_ERROR;
    }
    if (tscf->deprecated_oper_log == NULL) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "must assign deprecated oper log path, "
                           "use directives \"tfs_deprecated_oper_log\" ");
        return NGX_CONF_ERROR;
    }
    if (tscf->forbidden_oper_log == NULL) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "must assign forbidden oper log path, "
                           "use directives \"tfs_forbidden_oper_log\" ");
        return NGX_CONF_ERROR;
    }
