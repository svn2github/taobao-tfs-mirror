...
ngx_http_tfs_parse_login_message(ngx_http_tfs_t *t)
{
    ...
    rc_info = ngx_http_tfs_rcs_lookup(t->data, rc_ctx, t->r_ctx.appkey);
    ...
    if (rc == NGX_OK) {
        ...
        if (rc == NGX_ERROR) {
            ngx_log_error(NGX_LOG_ERR, t->log, 0,
                          "invalid session id: %V", &t->rc_info_node->session_id);
        }
    }
    return rc;
}
    ...
    ngx_http_tfs_rc_ctx_t *rc_ctx,  u_char *data)
{
    ...
    for (i = 0; i < rc_info_node->logical_cluster_count; i++) {
        ...
        for (j = 0; j < logical_cluster->rw_cluster_count; j++) {
            ...
            ngx_memcpy(physical_cluster->cluster_id_text.data,
                       p, physical_cluster->cluster_id_text.len);
            ...
            ngx_memcpy(physical_cluster->ns_vip_text.data,
                       p, physical_cluster->ns_vip_text.len);
