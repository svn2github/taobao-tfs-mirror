...
ngx_http_tfs_set_output_file_name(ngx_http_tfs_t *t)
{
    ...
    if (t->r_ctx.simple_name && t->r_ctx.file_suffix.data != NULL) {
...
ngx_http_tfs_alloc_st(ngx_http_tfs_t *t)
{
    ...
    } else if (t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_GET_OBJECT){
...
ngx_int_t
ngx_http_tfs_name_to_kv_init_meta(ngx_str_t *ctx_bucket_name,
    ngx_str_t *ctx_object_name, ngx_str_t *bucket_name,
    ngx_str_t *object_name)
{

    bucket_name->len = ctx_bucket_name->len;
    bucket_name->data = ctx_bucket_name->data;
    if (bucket_name->data == NULL) {
        return NGX_ERROR;
    }

    /* skip the initial "/" */
    object_name->len = ctx_object_name->len;
    object_name->data = ctx_object_name->data;
    if (object_name->data == NULL) {
        return NGX_ERROR;
    }
    ...
    ngx_log_t *log)
{
    ngx_log_debug7(NGX_LOG_DEBUG_HTTP, log, 0,
                   "=========dump segment data=========\n"
                   "block id: %uD, file id: %uL, offset: %L, size: %uL, "
                   "crc: %uD, oper_offset: %uD, oper_size: %uL",
...
void
ngx_http_tfs_dump_custom_file_info(ngx_http_tfs_custom_file_info_t *file_info,
    ngx_log_t *log)
{
    ngx_log_debug6(NGX_LOG_DEBUG_HTTP, log, 0,
                   "=========dump custom file info=========\n"
                   "name: %V, pid: %L, id: %L, create_time: %uD, "
                   "modify_time: %uD, size: %uL",
                   &file_info->name, file_info->pid, file_info->id,
                   file_info->create_time, file_info->modify_time,
                   file_info->size);
}


void
ngx_http_tfs_dump_object_meta_info(ngx_http_tfs_object_meta_info_t *meta_info,
    ngx_log_t *log)
{
    ngx_log_debug5(NGX_LOG_DEBUG_HTTP, log, 0,
                   "=========dump object meta info=========\n"
                   "create_time: %L, modify_time: %L, max_tfs_file_size: %uD, "
                   "big_file_size: %uL, owner_id: %uL",
                   meta_info->create_time, meta_info->modify_time,
                   meta_info->max_tfs_file_size, meta_info->big_file_size,
                   meta_info->owner_id);
}


void
ngx_http_tfs_dump_tfs_file_info(ngx_http_tfs_file_info_t *tfs_file_info,
    ngx_log_t *log)
{
    ngx_log_debug5(NGX_LOG_DEBUG_HTTP, log, 0,
                   "=========dump tfs file info=========\n"
                   "cluster id: %L, block id: %uD, file id: %uL, offset: %L, "
                   "size: %uL",
                   tfs_file_info->cluster_id, tfs_file_info->block_id,
                   tfs_file_info->file_id, tfs_file_info->offset,
                   tfs_file_info->size);
}


void
ngx_http_tfs_dump_object_info(ngx_http_tfs_object_info_t *object_info, ngx_log_t *log)
{
    size_t  i;

    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, log, 0, "=========dump object info========="
                   "has_meta_info: %uD, has_customize_info: %uD, tfs_file_count: %uD",
                   object_info->has_meta_info, object_info->has_customize_info,
                   object_info->tfs_file_count);
    if (object_info->has_meta_info) {
        ngx_http_tfs_dump_object_meta_info(&object_info->meta_info, log);
    }
    for (i = 0; i < object_info->tfs_file_count; i++) {
        ngx_http_tfs_dump_tfs_file_info(&object_info->tfs_file_infos[i], log);
    }
}
