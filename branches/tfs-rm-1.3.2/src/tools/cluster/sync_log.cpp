/*
 * (C) 2007-2010 Alibaba Group Holding Limited.
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 *
 *
 * Version: $Id$
 *
 * Authors:
 *   chuyu <chuyu@taobao.com>
 *      - initial release
 *
 */
#include <stdio.h>
#include <stdlib.h>
#include <tbsys.h>
#include <vector>
#include <string>
#include <sys/types.h>
#include <dirent.h>

#include "common/func.h"
#include "common/directory_op.h"
#include "client/fsname.h"
#include "client/tfs_client_api.h"

using namespace std;
using namespace tfs::client;
using namespace tfs::common;

struct SyncStat
{
  int64_t total_count_;
  int64_t success_count_;
  int64_t fail_count_;
};
struct SyncStat sync_stat;
int get_file_list(const string& log_file, vector<string>& name_vec);
int sync_file(vector<string>& name_vec);
int cmp_file_info(string& file_name);
char* str_match(char* data, const char* prefix);

static const int32_t MAX_READ_SIZE = 81920;
static const int32_t MAX_READ_LEN = 256;
TfsClient source_tfs_client_;
TfsClient dest_tfs_client_;
string source_ns_ip_ = "", dest_ns_ip_ = "";

static void usage(const char* name)
{
  fprintf(stderr, "Usage: %s -s -d -f [-g] [-l] [-h]\n", name);
  fprintf(stderr, "       -s source ns ip port\n");
  fprintf(stderr, "       -d dest ns ip port\n");
  fprintf(stderr, "       -f file name, assign a file to sync\n");
  fprintf(stderr, "       -g log file name, redirect log info to log file, optional\n");
  fprintf(stderr, "       -l log file level, set log file level, optional\n");
  fprintf(stderr, "       -h help\n");
  exit(TFS_ERROR);
}

int main(int argc, char* argv[])
{
  int32_t i;
  string file_path = "./";
  string file_name = "";
  string log_file = "./log/sync_report.log", level = "info";
  // analyze arguments
  while ((i = getopt(argc, argv, "s:d:f:g:l:h")) != EOF)
  {
    switch (i)
    {
    case 's':
      source_ns_ip_ = optarg;
      break;
    case 'd':
      dest_ns_ip_ = optarg;
      break;
    case 'f':
      file_name = optarg;
      break;
    case 'g':
      log_file = optarg;
      break;
    case 'l':
      level = optarg;
      break;
    case 'h':
    default:
      usage(argv[0]);
    }
  }

  if ((source_ns_ip_.empty())
      || (source_ns_ip_.compare(" ") == 0)
      || dest_ns_ip_.empty()
      || (dest_ns_ip_.compare(" ") == 0)
      || file_name.empty()
      || (file_name.compare(" ") == 0))
  {
    usage(argv[0]);
  }
  if ((level != "info") && (level != "debug") && (level != "error") && (level != "warn"))
  {
    fprintf(stderr, "level(info | debug | error | warn) set error\n");
    return TFS_ERROR;
  }
  if (log_file.size() != 0 && access(log_file.c_str(), R_OK) == 0)
  {
    char old_log_file[256];
    sprintf(old_log_file, "%s.%s", log_file.c_str(), Func::time_to_str(time(NULL), 1).c_str());
    rename(log_file.c_str(), old_log_file);
  }
  else if (!DirectoryOp::create_full_path(log_file.c_str(), true))
  {
    TBSYS_LOG(ERROR, "create file(%s)'s directory failed", log_file.c_str());
    return TFS_ERROR;
  }

  TBSYS_LOGGER.setFileName(log_file.c_str(), true);
  TBSYS_LOGGER.setMaxFileSize(1024 * 1024 * 1024);
  TBSYS_LOGGER.setLogLevel(level.c_str());

  source_tfs_client_.initialize(source_ns_ip_.c_str());
  dest_tfs_client_.initialize(dest_ns_ip_.c_str());

  memset(&sync_stat, 0, sizeof(sync_stat));
  vector<string> name_vec;
  name_vec.clear();

  get_file_list(file_name, name_vec);
  if (name_vec.size() > 0)
  {
    sync_file(name_vec);
    sync_stat.total_count_ = name_vec.size();
  }

  fprintf(stdout, "TOTAL COUNT: %"PRI64_PREFIX"d, SUCCESS COUNT: %"PRI64_PREFIX"d, FAIL COUNT: %"PRI64_PREFIX"d\n",
      sync_stat.total_count_, sync_stat.success_count_, sync_stat.fail_count_);
  fprintf(stdout, "LOG FILE: %s\n", log_file.c_str());

  return TFS_SUCCESS;
}
int get_file_list(const string& log_file, vector<string>& name_vec)
{
  int ret = TFS_ERROR;
  FILE* fp = fopen (log_file.c_str(), "r");
  if (fp == NULL)
  {
    TBSYS_LOG(ERROR, "open file(%s) fail,errors(%s)", log_file.c_str(), strerror(errno));
  }
  else
  {
    char info[MAX_READ_LEN];
    while(!feof(fp))
    {
      memset(info, 0, MAX_READ_LEN);
      fgets (info, MAX_READ_LEN, fp);
      if (strlen(info) > 0)
      {
        char* block = str_match(info, "blockid: ");
        uint32_t block_id = static_cast<uint32_t>(atoi(block));
        char* file = str_match(block + strlen(block) + 1, "fileid: ");
        uint64_t file_id = strtoul(file, NULL, 10);
        FSName fs;
        fs.set_block_id(block_id);
        fs.set_file_id(file_id);
        name_vec.push_back(string(fs.get_name()));
        TBSYS_LOG(INFO, "block_id: %u, file_id: %"PRI64_PREFIX"u, name: %s\n", block_id, file_id, fs.get_name());
      }
    }
    fclose (fp);
    ret = TFS_SUCCESS;
  }
  return ret;
}

int sync_file(vector<string>& name_vec)
{
  int ret = TFS_SUCCESS;
  vector<string>::iterator iter = name_vec.begin();
  for (; iter != name_vec.end(); iter++)
  {
    int ret = source_tfs_client_.tfs_open((*iter).c_str(), NULL, READ_MODE);
    if (ret == TFS_SUCCESS)
    {
      int32_t total_len = 0;
      // when dest file is different, write new file to dest.
      if (cmp_file_info(*iter) != 0)
      {
        ret = dest_tfs_client_.tfs_open((*iter).c_str(), NULL, WRITE_MODE | NEWBLK_MODE);
        char data[MAX_READ_SIZE];
        int32_t rlen = 0;

        for (;;)
        {
          FileInfo file_info;
          rlen = source_tfs_client_.tfs_read_v2(data, MAX_READ_SIZE, &file_info);
          if (rlen < 0)
          {
            fprintf(stderr, "read tfsfile fail\n");
            ret = TFS_ERROR;
            break;
          }
          if (rlen == 0)
          {
            break;
          }
          total_len += rlen;
          if (dest_tfs_client_.tfs_write(data, rlen) != rlen)
          {
            fprintf(stderr, "write tfsfile fail\n");
            ret = TFS_ERROR;
            break;
          }
        }
      }
      if (ret == TFS_SUCCESS && (dest_tfs_client_.tfs_close()))
      {
        TBSYS_LOG(INFO, "sync file(%s) from(%s) to dest(%s) success.", (*iter).c_str(),
            source_ns_ip_.c_str(), dest_ns_ip_.c_str());
        sync_stat.success_count_++;
      }
      else
      {
        TBSYS_LOG(ERROR, "sync file(%s) from(%s) to dest(%s) failed.", (*iter).c_str(),
            source_ns_ip_.c_str(), dest_ns_ip_.c_str());
        sync_stat.fail_count_++;
      }
      source_tfs_client_.tfs_close();
    }
    else
    {
      TBSYS_LOG(ERROR, "get file(%s) from source ns error", (*iter).c_str());
    }
  }
  return ret;
}

int cmp_file_info(string& file_name)
{
  int ret = 1;
  FileInfo source_buf;
  FileInfo dest_buf;
  int iret = dest_tfs_client_.tfs_open(file_name.c_str(), NULL, READ_MODE);
  if (iret == TFS_SUCCESS)
  {
    iret = dest_tfs_client_.tfs_stat(&dest_buf, READ_MODE);
    if (iret == TFS_SUCCESS)
    {
      iret = source_tfs_client_.tfs_stat(&source_buf, READ_MODE);
      if (iret == TFS_SUCCESS)
      {
        if ((dest_buf.id_ == source_buf.id_)
            && (dest_buf.size_ == source_buf.size_)
            && (dest_buf.flag_ == source_buf.flag_)
            && (dest_buf.crc_ == source_buf.crc_))
        {
          TBSYS_LOG(INFO, "file_id(%"PRI64_PREFIX"u) exists", source_buf.id_);
          ret = 0;
        }
      }
    }
    dest_tfs_client_.tfs_close();
  }
  return ret;
}

char* str_match(char* data, const char* prefix)
{
  if (data == NULL || prefix == NULL)
  {
    return NULL;
  }
  char* pch = strstr(data, prefix);
  if (pch != NULL)
  {
    pch += strlen(prefix);
    char* tmp = strchr(pch , ',');
    *tmp = '\0';
  }

  return pch;
}
