...
namespace tfs
{
  namespace client
  {
    ...
    private:
      ...
      template<class T> int do_unlink(const T& seg_info, const char* addr);
    ...
    template<class T> int GcWorker::do_gc_ex(T& meta, const char* file_name, const char* addr)
    {
      ...
      else if ((ret = do_unlink(meta.get_seg_info(), addr)) != common::TFS_SUCCESS)
      {
    ...
    template<class T> int GcWorker::do_unlink(const T& seg_info, const char* addr)
    {
      int ret = common::TFS_SUCCESS;
      ...
      for (typename T::const_iterator it = seg_info.begin(); it != seg_info.end(); ++it)
      {
        ...
        if (tfs_client_->unlink(fsname.get_name(), NULL, addr) != common::TFS_SUCCESS)
        {
