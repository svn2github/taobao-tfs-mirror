...
namespace tfs
{
  namespace dataserver
  {
   ...
   int TfsMirrorBackup::remove_file(const uint64_t block_id, const uint64_t file_id,
    ...
    TfsOldMirrorBackup::TfsOldMirrorBackup(SyncBase& sync_base, const char* src_addr, const char* dest_addr):
        sync_base_(sync_base), do_sync_mirror_thread_(0), do_fail_sync_mirror_thread_(0),
        tfs_client_(NULL), client_init_flag_(false)
    {
      if (NULL != src_addr &&
    ...
    TfsOldMirrorBackup::~TfsOldMirrorBackup()
    {
    }

    bool TfsOldMirrorBackup::init()
    {
      bool ret = (strlen(src_addr_) > 0 && strlen(dest_addr_) > 0) ? true : false;
      if (ret)
      {
        tfs_client_ = client::TfsClientImpl::Instance();
        client_init_flag_ =
          tfs_client_->initialize(NULL, DEFAULT_BLOCK_CACHE_TIME, 0, false) == TFS_SUCCESS ?
          true : false;
        TBSYS_LOG(INFO, "TfsSyncMirror init %s. source ns addr: %s, destination ns addr: %s", client_init_flag_ ? "success" : "fail", src_addr_, dest_addr_);
        if (do_sync_mirror_thread_ == 0)
    ...
    void TfsOldMirrorBackup::destroy()
    {
      if (0 != do_sync_mirror_thread_)
      {
        do_sync_mirror_thread_->join();
    ...
    inline BlockManager& TfsOldMirrorBackup::get_block_manager()
    {
      return dynamic_cast<DataService*>(DataService::instance())->get_block_manager();
    }

    int TfsOldMirrorBackup::do_sync(const SyncData *sf)
    {
      int ret = TFS_SUCCESS;
      if (!client_init_flag_)
      {
    ...
    int TfsOldMirrorBackup::remote_copy_file(const uint32_t block_id, const uint64_t file_id)
    {
    ...
    int TfsOldMirrorBackup::copy_file(const uint32_t block_id, const uint64_t file_id)
    {
      int32_t ret = block_id > 0 ? TFS_SUCCESS : EXIT_BLOCKID_ZERO_ERROR;
      if (TFS_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          if (TFS_SUCCESS != ret)
          {
            TBSYS_LOG(WARN, "stat file %s fail. blockid: %u, fileid: %"PRI64_PREFIX"u,ret: %d",
                fsname.get_name(), block_id, file_id, ret);
          }
          else
          {
            ...
            finfo.size_ -= FILEINFO_EXT_SIZE; // ignore first 4 bytes
            ...
            if (TFS_SUCCESS == ret)
            {
              while (total_length < finfo.size_ && TFS_SUCCESS == ret)
              {
                ...
                ret = get_block_manager().read(data, length, offset, file_id,
                    READ_DATA_OPTION_FLAG_FORCE, block_id, block_id);
    ...
    int TfsOldMirrorBackup::rename_file(const uint32_t block_id, const uint64_t file_id,
