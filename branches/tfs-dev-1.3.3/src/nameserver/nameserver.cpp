...
namespace tfs
{
  namespace nameserver
  {
    ...
    int NameServer::start()
    {
      ...
      NewClientManager::get_instance().initialize_with_transport(&transport_);
...
PASS:
    ...
    int NameServer::get_peer_role()
    {
      ...
      while (true)
      {
        ...
        ret = message::NewClientManager::get_instance().call(ngi.other_side_ip_port_, &master_slave_msg, DEFAULT_NETWORK_CALL_TIMEOUT, ret_msg);
        if (ret != TFS_SUCCESS)
        {
          ret = EXIT_GENERAL_ERROR;
          goto out;
