...
int send_message_to_server(uint64_t server, NewClient* client, Message* msg, string& err_msg, Message** output = NULL/*not free*/);
...
int repair_lose_block(TfsClient& tfs_client, VSTRING& param)
{
  ...
  req_cc_msg.set_value2(src_server_id);
  req_cc_msg.set_value1(dest_server_id);
  string err_msg;
  NewClient* client = NewClientManager::get_instance().create_client();
  if (send_message_to_server(nsip_port, client, &req_cc_msg, err_msg) == TFS_ERROR)
  {
...
int main(int argc,char** argv)
{
  ...
  int iret = tfs_client.initialize(ns_ip_port);
  if (iret != TFS_SUCCESS)
  {
    return TFS_ERROR;
  }
  ...
  NewClientManager::get_instance().initialize();
