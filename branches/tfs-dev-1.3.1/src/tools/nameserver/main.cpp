...
int parse_block_param(VSTRING& param, int8_t& flag, int32_t& num, uint32_t& block_id, int32_t& count, int32_t& interval)
{
  flag = CMD_NOP;
    ...
    else
    {
      ...
      switch (cmd)
      {
        ...
        case CMD_BLOCK_ID:
          ...
          break;
        case CMD_COUNT:
          count = static_cast<uint32_t>(atoi((*(++iter)).c_str()));
          ...
          break;
        case CMD_INTERVAL:
          interval = static_cast<uint32_t>(atoi((*(++iter)).c_str()));
          if (interval < 0 || interval == INT_MAX || interval == INT_MIN)
          {
            TBSYS_LOG(ERROR, "unvalid param (cmd---%d)", cmd);
            exit(TFS_ERROR);
          }
...
int parse_server_param(VSTRING& param, int8_t& flag, int32_t& num, int32_t& count, int32_t& interval)
{
  flag = CMD_NOP;
  VSTRING::iterator iter = param.begin();
  for (; iter != param.end(); iter++)
...
int parse_machine_param(VSTRING& param, int8_t& flag,  int32_t& count, int32_t& interval)
{
  flag = PRINT_ALL;
  VSTRING::iterator iter = param.begin();
  for (; iter != param.end(); iter++)
...
void init()
{
  ...
  g_cmd_map["block"] = CmdNode("block  [-n num] [-id block_id] [-s] [-c] [-i]   show block info\n -s print server list ", 0, 5, cmd_show_block);
  g_cmd_map["server"] = CmdNode("server [-n num] [-b] [-w] [-m] [-c] [-i]   show server info\n -b print block list \n -w print writable block list \n -m print master block list", 0, 5, cmd_show_server);
  g_cmd_map["machine"] = CmdNode("machine [-a] [-p] [-c] [-i]   show machine info \n -a print all info \n -p print part of infos ", 0, 5, cmd_show_machine);
  ...
  g_sub_cmd_map["-count"] = CMD_COUNT;
  g_sub_cmd_map["-interval"] = CMD_INTERVAL;

  g_sub_cmd_map["-n"] = CMD_NUM;
  g_sub_cmd_map["-d"] = CMD_BLOCK_ID;
  ...
  g_sub_cmd_map["-c"] = CMD_COUNT;
  g_sub_cmd_map["-i"] = CMD_INTERVAL;
}
...
int cmd_show_block(VSTRING& param)
{
  ...
  int32_t count = 1;
  int32_t interval = 0;
  parse_block_param(param, flag, num, block_id, count, interval);
  g_show_info.show_block(flag, num, block_id, count, interval);
  return TFS_SUCCESS;
}
...
int cmd_show_server(VSTRING& param)
{
  ...
  int32_t count = 1;
  int32_t interval = 0;
  parse_server_param(param, flag, num, count, interval);
  g_show_info.show_server(flag, num, count, interval);
  return TFS_SUCCESS;
}
...
int cmd_show_machine(VSTRING& param)
{
  ...
  int32_t count = 1;
  int32_t interval = 0;
  parse_machine_param(param, flag, count, interval);
  g_show_info.show_machine(flag, num, count, interval);
  return TFS_SUCCESS;
}
