...
//int get_diff_block();
int get_file_list(const string& src_ns_addr, const uint32_t block_id, BLOCK_FILE_INFO_VEC& v_block_file_info);
int get_file_list(const string& ns_addr, const uint32_t block_id, FILE_NAME_VEC& v_file_name);
int sync_file(const string& src_ns_addr, const string& dest_ns_addr, BLOCK_FILE_INFO_VEC& v_block_file_info, const bool force, const int32_t modify_time);
...
class WorkThread : public tbutil::Thread
{
  public:
    WorkThread(const string& src_ns_addr, const string& dest_ns_addr, const bool need_remove_file, const bool force, const int32_t modify_time):
      src_ns_addr_(src_ns_addr), dest_ns_addr_(dest_ns_addr), need_remove_file_(need_remove_file), force_(force), modify_time_(modify_time)
  {
    ...
    virtual void run()
    {
      if (!destroy_)
      {
        ...
        {
          get_file_list(src_ns_addr_, (*iter), v_block_file_info);
          sync_file(src_ns_addr_, dest_ns_addr_, v_block_file_info, force_, modify_time_);
          if (need_remove_file_)
          {
            FILE_NAME_VEC src_v_file_name, dest_v_file_name, diff_v_file_name;

            get_file_list(src_ns_addr_, (*iter), src_v_file_name);
            TBSYS_LOG(DEBUG, "source file list size: %zd", src_v_file_name.size());

            get_file_list(dest_ns_addr_, (*iter), dest_v_file_name);
...
{
  fprintf(stderr, "Usage: %s -s -d -b -e -u [-g] [-l] [-h]\n", name);
...
int sync_file(const string& src_ns_addr, const string& dest_ns_addr, BLOCK_FILE_INFO_VEC& v_block_file_info, const bool force, const int32_t modify_time)
{
  ...
  for (; iter != v_block_file_info.end(); iter++)
  {
    BlockFileInfo block_file_info = (*iter);
    SyncAction sync_action;

    // compare file info
    ret = sync_op.cmp_file_info(block_file_info.block_id_, block_file_info.file_info_, sync_action, force, modify_time);

    FSName fsname(block_file_info.block_id_, block_file_info.file_info_.id_, TfsClientImpl::Instance()->get_cluster_id(src_ns_addr.c_str()));
