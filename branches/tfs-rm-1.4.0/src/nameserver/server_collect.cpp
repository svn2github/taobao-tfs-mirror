...
namespace nameserver
{
  ...
  ServerCollect::ServerCollect(const DataServerStatInfo& info, time_t now):
    GCObject(now),
  ...
  bool ServerCollect::add(BlockCollect* block, bool writable)
  {
  ...
  bool ServerCollect::clear(LayoutManager& manager, time_t now)
  {
    bool remove = false;
  ...
  int ServerCollect::scan(SSMScanParameter& param, int8_t scan_flag)
  {
    RWLock::Lock lock(*this, READ_LOCKER);
   ...
   */
  bool  ServerCollect::is_writable(int64_t average_used_capacity) const
  {
    if (is_full())
    {
    ...
    if (use_capacity_ > (average_used_capacity * AVERAGE_USED_CAPACITY_MULTIPLE))
    {
  ...
  bool ServerCollect::is_readable(int32_t average_load) const
  {
  ...
  void ServerCollect::update(const common::DataServerStatInfo& info, time_t now)
  {
    RWLock::Lock lock(*this, WRITE_LOCKER);
    id_ = info.id_;
    use_capacity_ = info.use_capacity_;
  ...
  bool ServerCollect::can_be_master(int32_t max_write_block_count)
  {
    RWLock::Lock lock(*this, READ_LOCKER);
