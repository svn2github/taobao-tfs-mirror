...
public class NameServerPlanTestCase {
	...
	final public int WRITE_TIME = 1800;     //in second
...
//				break;
	 ...
	 * @return
	 */
	public boolean checkNetworkTrafBalance()
	{	
		boolean bRet = false;
		
		String strCmd = "sar -n DEV -f " + SAR_DSA_LOG + 
			"grep | avarage | grep eth0 | awk 'print $" + RXBYTPERSEC_SD_COL + "'";
		ArrayList<String> output = new ArrayList<String>();
		bRet = Proc.proStartBase(DSIPA, strCmd, output);
		if (bRet == false) return bRet;
		
		if (output.size() == 0)
			return false;
		float rxbytPerSecDSA = Float.parseFloat(output.get(0));	
		log.debug("rxbytPerSecDSA: " + output.get(0));
		
		strCmd = "sar -n DEV -f " + SAR_DSB_LOG + 
			"grep | avarage | grep eth0 | awk 'print $" + RXBYTPERSEC_SD_COL + "'";
		output.clear();
		bRet = Proc.proStartBase(DSIPB, strCmd, output);
		if (bRet == false) return bRet;		
		
		if (output.size() == 0)
			return false;
		float rxbytPerSecDSB = Float.parseFloat(output.get(0));	
		log.debug("rxbytPerSecDSB: " + output.get(0));
		
		float temp;
		if (rxbytPerSecDSA < rxbytPerSecDSB)
		{
			temp = rxbytPerSecDSA;
			rxbytPerSecDSA = rxbytPerSecDSB;
			rxbytPerSecDSB = temp;
		}
		
		float diff = rxbytPerSecDSA - rxbytPerSecDSB;
